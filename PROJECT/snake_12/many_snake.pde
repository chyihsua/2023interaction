...
void hit() {
  ...
  for (int i=0; i<min-1; i++) {
    ...
    if (x<=sizeX+sizeSnake/5 || y<=sizeY+sizeSnake/5 || x>=sizeX+3000-sizeSnake/5 || y>=sizeY+2400-sizeSnake/5) { //IndexOutOfBoundsException: Index 16 out of bounds for length 16
    status=2;
  }
}
