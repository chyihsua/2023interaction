...
class Snake {
  ...
  boolean check(float x, float y, int min) {
...
void hit() {
  int min=body.size();
  int[] size={body.size(), snake2.body.size(), snake3.body.size(), snake4.body.size(), snake5.body.size()};
  for (int i=0; i<size.length; i++) {
    if (size[i]<min) min=size[i];
  }

  if (snake2.check(x, y, min) || snake3.check(x, y, min) || snake4.check(x, y, min) || snake5.check(x, y, min)) {
    status=2;
  }
